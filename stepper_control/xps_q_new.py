...
class XPS:
    ...
    def __sendAndReceive(self, socketId, command):
        try:
            ...
            # Keep receiving until EndOfAPI is found
            # Note: we check against the bytes version b',EndOfAPI'
            while b',EndOfAPI' not in ret:
                ret += XPS.__sockets[socketId].recv(1024)

        except socket.timeout:
            return [-2, 'Timeout Error']
        except Exception as e:
            return [-3, f'Socket Error: {str(e)}']

        # Convert bytes to string once to make parsing easy
        response_str = ret.decode('utf-8')

        # Expected wire format:
        #   <error_code>,<payload>,EndOfAPI
        # where <payload> may itself contain commas.
        first_comma = response_str.find(',')
        if first_comma < 0:
            return [-5, response_str]

        try:
            error_code = int(response_str[:first_comma])
        except ValueError:
            return [-4, 'Invalid Error Code Format']

        payload = response_str[first_comma + 1 :]
        end_marker = ',EndOfAPI'
    ...
    def PositionerUserTravelLimitsGet (self, socketId, PositionerName):
        ...
        # Some controllers return trailing commas/whitespace; parse defensively.
        text = str(returnedString).replace(';', ',')
        tokens = [tok.strip() for tok in text.split(',') if tok.strip() != '']
        if (len(tokens) < 2):
            return [-1, returnedString]

        try:
            user_min = float(tokens[0])
            user_max = float(tokens[1])
        except Exception:
            return [-1, returnedString]

        return [error, user_min, user_max]
